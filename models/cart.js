...
class Cart {
  constructor(email, snackId, quantity, snackName, snackPrice) {
    this.email = email;
  ...
  static async checkForCart(email) {
    ...
    return result.recordset.map(
      (cart) => new Cart(cart.Email, cart.SnackId, cart.Quantity)
    );
  }
  ...
  static async getCartContents(email) {
    const connection = await sql.connect(dbConfig);
    const sqlQuery = `
      SELECT c.Email, c.SnackId, c.Quantity, s.SnackName, s.SnackPrice 
      FROM Cart c
